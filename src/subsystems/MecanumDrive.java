...
public class MecanumDrive {
  private Talon frontLeft, backLeft;
  ...
  public void plainDrive() {
    driveTrain.mecanumDrive_Cartesian(Robot.driver.getx(), Robot.driver.gety(),
      Robot.driver.getAxis(Constants.AXIS_RS_X), 0);
  }
  ...
  public void run() {
    ...
    if (!Constants.CALIBRATION_MODE) {
      ...
      if (false) {// * if(PID_IN_USE) {
        boolean driftingFromTurn = false;
        if (driverInput != 0) {
      ...
      if (checkPOV(spin, Constants.MANUAL_DPAD_SLOW)) {
      } // scans for dpad input and does it in the method where it checks via switch statement.
  ...
  public boolean nearSetpointMoving() {
    return (Math.abs(rotationPID.getError()) < Constants.DRIVE_PID_ERROR_THRESHOLD_MOVING);
  }
  ...
  void setDampening(boolean state) {
    dampening = state;
  }
