...
public class AutonRunner {
  ...
  public void runPeriodic() {
    ...
    if (!broken) {
      Robot.mDrive.driveTrain.mecanumDrive_Cartesian(-driverX.updateOutput(Robot.encoders.encoderX.getDistance()),
        driverY.updateOutput(Robot.encoders.encoderY.getDistance()), 0,
  ...
  private boolean atTarget(boolean useTimer) {
    if (useTimer) {
      if (!(Math.abs(driverX.getError()) < Constants.AUTON_DRIVE_ERROR_THRESHOLD && Math.abs(
        driverY.getError()) < Constants.AUTON_DRIVE_ERROR_THRESHOLD)) {
        errorTimer.reset();
      }
